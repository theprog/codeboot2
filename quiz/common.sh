...
# see : https://marc.waeckerlin.org/computer/blog/parsing_of_query_string_in_bash_cgi_scripts
...
# For all given query strings parse them an set shell variables
function setQueryVars() {
  ...
  for var in ${vars//&/ }; do                  # split at &
    ...
    eval "post_${var%=*}=\"${value//\"/\\\"}\"" # evaluate assignment
  done
}
